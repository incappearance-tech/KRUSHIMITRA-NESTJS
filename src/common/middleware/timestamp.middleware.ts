...
export class TimestampMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    const url = req.url || '';
    if (req.method === 'GET' || url.includes('/health') || url.includes('/payments/webhook')) {
      return next();
    }
    ...
    if (timeDiff > this.TIMESTAMP_TOLERANCE_MS) {
      this.logger.warn(
        `Request timestamp out of range: ${timeDiff}ms from ${req.ip}`,
      );
      throw new BadRequestException(
        'Request timestamp expired or too far in future',
      );
    }
    ...
    if (nonceExists) {
      this.logger.error(
        `Replay attack detected! Nonce reused: ${nonce} from ${req.ip}`,
      );
      throw new BadRequestException(
        'Request nonce already used (replay attack detected)',
      );
    }
    ...
    this.logger.debug(
      `Request validated: timestamp=${requestTime}, nonce=${nonce.substring(0, 8)}...`,
    );
    next();
  }
}
